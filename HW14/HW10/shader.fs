...
// pseudo random 함수
float random(int i){
    float dot_product = dot(vec4(gl_FragCoord.xyz, float(i)),
...
void main(void)
{
    ...
    vec3 diffuse  = baseColor * max(dot(N, L), 0.0) * lightColor;
    vec3 specular = lightColor * pow(max(dot(R, V), 0.0), shininess);
    vec3 C = diffuse + specular;
    ...
    // 64개 중 16개를 픽셀마다 랜덤 선택
    for(int i=0; i<16; i++){
        int index = int(random(i) * 64.0) % 64;

       
        vec2 offset = poissonDisk64[index] / 200.0;

        if( texture(shadowTex, sCoord.xy + offset).r < (sCoord.z - bias) ){
            visibility -= 0.0625;
        }
    }
    ...
    C *= visibility; //gamma transform 전에 적용
