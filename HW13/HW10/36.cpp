...
int main(void){
    ...
    programID = loadShaders(
        "/Users/emfpdlzj/Desktop/Computer-Graphics/HW13/HW10/shader.vs",
        "/Users/emfpdlzj/Desktop/Computer-Graphics/HW13/HW10/shader.fs"
    );
    shadowProgram = loadShaders(
        "/Users/emfpdlzj/Desktop/Computer-Graphics/HW13/HW10/shadow.vs",
        "/Users/emfpdlzj/Desktop/Computer-Graphics/HW13/HW10/shadow.fs"
    );

    loadJ3A("/Users/emfpdlzj/Desktop/Computer-Graphics/HW13/HW10/dwarf.j3a");
    ...
    std::string basePath = "/Users/emfpdlzj/Desktop/Computer-Graphics/HW13/HW10/";
...
void render(GLFWwindow* window){
    ...
    glUseProgram(shadowProgram);                           // shadowProgram을 use 하고 uniform으로 shadowMVP를 보냄
    mat4 shadowMVP = shadowProjMat * shadowViewMat;
    GLint locShadowMVP = glGetUniformLocation(shadowProgram,
                                              "shadowMVP");
    glUniformMatrix4fv(locShadowMVP, 1, GL_FALSE,
                       value_ptr(shadowMVP));

    model.draw(); //model과 floorModel을 그림
    floorModel.draw();

    // 원래 rendering을 진행하기 위해 framebuffer를 GL_NONE으로 지정
    glBindFramebuffer(GL_FRAMEBUFFER, GL_NONE);

    //viewport를 화면 영역으로 다시 설정
    int width, height;
    glfwGetFramebufferSize(window, &width, &height);
    glViewport(0, 0, width, height);

    //기존과 같이 각종 행렬을 계산해서 이전에 사용하던 program을 use 한 후에 uniform을 보내는 과정
    glClearColor(0.2f, 0.7f, 0.2f, 1.0f);
    ...
    //fragment shader에 정수로 된 uniform 변수 diffTexEnabled 와 bumpTexEnabled
    ...
    //main 파일의 rendering 부분에 model을 draw하기 직전에 uniform 값인 bumpTexEnabled와 diffTexEnabled를 1로 지정하고 draw
    glUniform1i(locDiffTexEnabled, 1);
    glUniform1i(locBumpTexEnabled, 1);
    glUniform3fv(locColor, 1, &diffuseColor[0].x);
    model.draw(); // rendering 과정에서 buffer, vertex array를 bind 하는 부분과 glDrawElements 를 지운 후 다음의 코드로 이 과정을 진행

    //바닥을 그리기 직전에 uniform 값인 bumpTexEnabled와 diffTexEnabled를 0으로 지정
    vec3 floorColor = vec3(0.85f, 0.85f, 0.85f); //이때 uniform인 color 값도 원하는 값(가급적 어둡지 않는 색)으로 지정
    glUniform1i(locDiffTexEnabled, 0);
    glUniform1i(locBumpTexEnabled, 0);
    glUniform3fv(locColor, 1, &floorColor[0]);
    floorModel.draw(); // Rendering 과정에서는 역시 floorModel.draw(); 를 통하여 바닥을 그릴 수 있음.
