...
uniform int diffTexEnabled = 1; // fragment shader에 정수로 된 uniform 변수 diffTexEnabled 와 bumpTexEnabled
uniform int bumpTexEnabled = 1;
uniform vec3 color = vec3(1.0, 0.5, 0.0); // 물체 색상을 지정하는 uniform 변수
...
void main(void)
{
    ...
    //    TBN을 계산하는 부분부터 bump mapping이 적용된 N을 계산하는 부분까지를 bumpTexEnabled가 0이 아닌 경우에만 수행되도록
    ...
    // diffTexEnabled가 0이 아니면 이 지역변수 값을 texture에서 얻어온 값으로 바꿈
    if( diffTexEnabled != 0 ){
    ...
    //  diffuse reflection을 계산하는 과정에 사용되는 color 값을 이 지역변수로 바꿈
