    ...
    out vec3 normal; // 2.normal vector를 fragment shader로 보내기 위해 out 변수

    void main(){
        //  n_TexCoord를 texCoord에 대입하여 fragment shader로 보내지도록 합니다.
        //texCoord = in_TexCoord;
        
        normal = normalize( (modelMat * vec4(in_Normal, 0.0)).xyz );
        //3,4: in_Normal에 0을 붙여 homogeneous coordinate(vec4)를 만들고
        //5: 이 vec4에 modelMat을 곱하여 world coordinate의 normal vector 방향을 계산
        //6:  xyz 성분만 떼내고 normalize() 함수를 이용해 길이를 1로 만든 후 normal (out변수)에 저장
        
        // homogeneous coordinate로 바꾸고 modelMat으로 world 좌표 계산
