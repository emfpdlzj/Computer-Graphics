...
int main(void){
    ...
    // 2.glBufferData 크기: nVertices[0]*sizeof(glm::vec3), data: normals[0]
    glBufferData(GL_ARRAY_BUFFER, nVertices[0]*sizeof(vec3), normals[0], GL_STATIC_DRAW);
    // 3.vertex array 생성 후에 attribute를 추가합니다. attribute 번호는 반드시 2번으로 하고, 개수는 3, type은 GL_FLOAT로 vertex와 동일합니다.
