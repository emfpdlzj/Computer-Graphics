...
// 1. mouse의 움직임을 알기 위해 직전의 pointer 위치를 저장할 전역변수(double) 선언
static double lastX = 0.0, lastY = 0.0;

void render(GLFWwindow* window);
void cursorCallback(GLFWwindow* window, double xpos, double ypos);                  // 5. 마우스 입력을 받기 위해 cursor position callback 함수
void buttonCallback(GLFWwindow* window, int button, int action, int mods);          // 2. 마우스가 눌린 순간 pointer의 위치를 lastX, lastY에 저장하기 위한 함수
...
// 6. main 밖에 cursorPosCB 함수 정의
void cursorCallback(GLFWwindow* window, double xpos, double ypos){
    // 8. 왼쪽 버튼이 눌려있는 동안에만 카메라를 움직
    if(glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS){
        // 7. 먼저 window의 크기를 얻어 옵니다
        int width = 1, height = 1;
        glfwGetFramebufferSize(window, &width, &height);

        // 9. theta = -(xpos - lastX)/width * π
        theta += (float)(-(xpos - lastX) / (double)width * 3.1415926535);

        // 10. phi  = -(ypos - lastY)/height * π
...
void buttonCallback(GLFWwindow* window, int button, int action, int mods){
    // 4. if문 속의 button == GLFW_MOUSE_BUTTON_LEFT 이고, action == GLFW_PRESS일 때
    if(button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS){
        // 2. 눌린 순간 pointer의 위치를 lastX, lastY에 저장
