    ...
    out vec3 worldPos; //1-1. out 변수로 vec3 worldPos를 선언합

    void main(){
        vec4 worldCoord = modelMat * vec4(in_Position, 1.0);  //1-2. in_Position을 homogeneous coordinate로 만들고, modelMat을 곱한 결과를 worldCoord
        worldPos = worldCoord.xyz; //1-3. worldCoord.xyz를 worldPos에 대입하여 fragment shader로 보냄
